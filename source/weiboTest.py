...
def spider(username, password, driver, book_name_xls, sheet_name_xls, keyword, maxWeibo):
    ...
    if os.path.exists(book_name_xls):
        ...
        print("文件不存在，重新创建")
        value_title = [["rid", "用户名称", "微博等级", "微博内容", "微博转发量",
                        "微博评论量", "微博点赞", "发布时间", "搜索关键词", "话题名称", "话题讨论数", "话题阅读数"], ]
...
if __name__ == '__main__':
    ...
    driver = webdriver.Chrome(
        '/Users/Desktop/python/weibo_keyword/chromedriver')  # 你的chromedriver的地址
    ...
    keywords = ["NBA模仿帝", ]  # 输入你想要的关键字，可以是多个关键词的列表的形式
    for keyword in keywords:
        spider(username, password, driver, book_name_xls,
